...
class SelfKittiDatasetStage1(Dataset):
    def __init__(
        self,
        data_root,
        split,
        voxel_filename='pseudo_packed',
    ):
        super().__init__()
        print(f"(wen_self_kitti_dataset1.py) 初始化 SemanticKittiDatasetStage1，資料根目錄: {data_root}, 資料集分割: {split}")
        self.dataset_type = 'SelfKittiDatasetStage1'
        self.data_root = data_root
        self.label_root = os.path.join(data_root, "labels")
        self.voxel_root = os.path.join(data_root, voxel_filename)
        self.n_classes = 2
        # splits = {
    ...
    def load_scans(self):
        ...
        glob_path = os.path.join(
                self.voxel_root, "*.pseudo"
            ) 
        
        print(f"載入voxels資料夾 {self.voxel_root} 下的所有 .pseudo 檔案，路徑模式： {glob_path}")
    ...
    def get_data_info(self, index):
        ...
        if self.split == "train" or self.split == "val":
            # load ground truth
            target_1_2_path = os.path.join(self.label_root, frame_id + "_1_2.npy")
            target = np.load(target_1_2_path)
            target = target.reshape(-1)
            target = target.reshape(128, 128, 16)
            target = target.astype(np.float32)
            
            target_1_4_path = os.path.join(self.label_root, frame_id + "_1_4.npy")
            ...
            target_1_8_path = os.path.join(self.label_root, frame_id + "_1_8.npy")
