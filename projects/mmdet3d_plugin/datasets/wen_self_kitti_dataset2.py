...
class SelfKittiDatasetStage2(Dataset):
    def __init__(
        self,
        data_root,
        semantickitti_yaml,
        ...
        with open(semantickitti_yaml, 'r') as stream:
    ...
    def load_poses(self):
        ...
        pose_path = os.path.join(self.data_root, "poses.txt")
        calib = self.read_calib(
            os.path.join(self.data_root, "calib.txt")
        )
        pose_list = self.parse_poses(pose_path, calib)
        return pose_list
    ...
    def load_scans(self):
        ...
        glob_path = os.path.join(
            self.query_root, "*." + self.query_tag
        )
        print(f"載入queries資料夾 {self.query_root} 的掃描檔案，路徑模式：{glob_path}")
    ...
    def get_data_info(self, index):
        ...
        target, target_2, target_4, target_8 = self.get_gt_info(frame_id)
    ...
    def get_meta_info(self, scan, frame_id, proposal_path):
        ...
        # for reference img
        seq_len = len(self.poses)
        # print(f"self.target_frames 數量為：{len(self.target_frames)}，若為 0 代表單幀模式，預設是單幀模式")
        for i in self.target_frames:
            if len(self.target_frames) == 1:
                ...
                # print("因為 target_frames 數量大於 2 ， 代表取用多幀影像，取用 image_2 資料夾裡該影像 target_frames範圍影像")
                id = int(frame_id)
    ...
    def get_input_info(self, frame_id):
        ...
        if self.split == "train" or self.split == "val":
            ...
            for i in self.target_frames:
                if len(self.target_frames) == 1:
                    ...
                    # openset_feat = torch.load(openset_feat_path).type(torch.float32) # wen commend
                    openset_feat = torch.zeros((1,1)) # wen add
    ...
    def get_gt_info(self, frame_id):
        ...
        if self.split == "train" or self.split == "val":
            ...
            target_1_path = os.path.join(self.label_root, frame_id + "_1_1.npy")
            target = np.load(target_1_path)
            target_2_path = os.path.join(self.label_root, frame_id + "_1_2.npy")
            target_2 = np.load(target_2_path)
            target_4_path = os.path.join(self.label_root, frame_id + "_1_4.npy")
            target_4 = np.load(target_4_path)
            target_8_path = os.path.join(self.label_root, frame_id + "_1_8.npy")
